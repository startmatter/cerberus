...
import type { CerberusConfig, CustomScanner, GatePolicy } from "./types.js";
...
function oneOf<T extends string>(
  ...
  throw new Error(
    `Invalid ${label}: "${String(v)}" (expected one of ${allowed.join(", ")})`,
  );
}
...
export function parseConfig(raw: unknown): CerberusConfig {
  ...
  const scanner = (key: string) =>
    isRecord(scanners[key]) ? (scanners[key] as Record<string, unknown>) : {};
  ...
  for (const entry of Array.isArray(scanners.custom) ? scanners.custom : []) {
    ...
    if (!name || !command)
      throw new Error("Custom scanners need both `name` and `command`");
    if (!command.includes("{output}"))
      throw new Error(
        `Custom scanner "${name}": command must contain {output}`,
      );
  ...
  return {
    scanners: {
      semgrep: {
        ...
        config:
          typeof semgrep.config === "string" && semgrep.config
            ? semgrep.config
            : "p/security-audit",
      ...
      trivy: {
        ...
        scanners: strArray(trivy.scanners).length
          ? strArray(trivy.scanners)
