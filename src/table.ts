...
function severityOf(result: Record<string, unknown>, rule?: Rule): string {
  ...
  const level =
    (result.level as string | undefined) ??
    rule?.defaultConfiguration?.level ??
    "warning";
  return level === "error" ? "high" : level === "warning" ? "medium" : "info";
}

/** Display form of a finding's location, e.g. `src/db.ts:42` or `-` when unknown. */
export function findingLocation(f: FlatFinding): string {
  if (!f.file) return "-";
...
export function flattenFindings(sarif: unknown): FlatFinding[] {
  ...
  for (const runRaw of Array.isArray(runs) ? runs : []) {
    const run = runRaw as {
      tool?: { driver?: { name?: string; rules?: Rule[] } };
      results?: unknown[];
    };
    ...
    for (const resRaw of Array.isArray(run.results) ? run.results : []) {
      ...
      const message = (
        (result.message as { text?: string } | undefined)?.text ?? ""
      )
        .split("\n")[0]!
        .trim();
      out.push({
  ...
  return out.sort(
    (a, b) => (RANK[b.severity] ?? -1) - (RANK[a.severity] ?? -1),
  );
}
...
export function renderTable(
  ...
  for (const f of findings) {
    ...
    const colored = useColor
      ? `${COLORS[f.severity] ?? ""}${sev}${RESET}`
      : sev;
    lines.push(
      `${colored}  ${f.tool.padEnd(10).slice(0, 10)}  ${findingLocation(f)}  ${f.title.slice(0, 100)}`,
    );
  }
